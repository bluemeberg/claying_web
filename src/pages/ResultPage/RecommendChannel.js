...
const RecommendChannel = () => {
  ...
  return (
    <Container>
      <NavBar back={true} />
      <ModalChannelBanner img={location.state.banner}>
      ...
      <ModalChannelThumbnail>
        {location.state.channelThumbnail !== undefined ? (
          <img
            src={location.state.channelThumbnail}
      ...
      {location.state.channelTitle !== undefined ? (
        <ChannelTitle>{location.state.channelTitle}</ChannelTitle>
      ) : (
        <ChannelTitle>{location.state.channelTitle}</ChannelTitle>
      )}
      <ChannelSubsCount>
        {location.state.subsDate !== undefined ? (
          <span> 구독중 </span>
        ) : (
          <p>미구독 </p>
        )}
        {location.state.subsCount !== undefined
          ? formatNumber(location.state.subsCount)
          : ""}
      </ChannelSubsCount>
      <ChannelVideoCount>
        동영상{" "}
        {location.state.videoCount !== undefined
          ? location.state.videoCount
          : location.state.videoCount}
        개
        {/* | 조회수{" "}
